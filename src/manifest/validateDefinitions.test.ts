...
function callValidateDefinitions(input: unknown): unknown {
  return Reflect.apply(validateDefinitions, undefined, [input]);
}
...
describe("validateDefinitions", () => {
  it("rejects invalid manifest containers", () => {
    expectManifestError(() => callValidateDefinitions(null), "MANIFEST_INVALID");
    expectManifestError(() => validateDefinitions({}), "MANIFEST_EMPTY");
  });
  ...
  it("rejects definitions that are not objects", () => {
    expectManifestError(
      () => callValidateDefinitions({ theme: null }),
      "DEFINITION_INVALID",
      "theme",
    );
    expectManifestError(
      () => callValidateDefinitions({ theme: [] }),
      "DEFINITION_INVALID",
      "theme",
    );
  });
  ...
  it.each([
    ...
    (_name, definition, code) => {
      expectManifestError(
        () => callValidateDefinitions({ preference: definition }),
        code,
        "preference",
      );
    },
